...
namespace wilson
{
	Scene11::Scene11(D3D11*const pD3D11)
	{
		m_bObject = false;
	...
	void Scene11::AddMeshEntity(Object11* const pObject, const UINT idx, const UINT meshIdx)
	{
		std::string name = pObject->GetName();

		Entity11* pEntity = new Entity11(name, idx, pObject, meshIdx);
		m_entites.push_back(pEntity);
	}
	void Scene11::AddLightEntity(Light11* const pLight, const std::string type, const UINT idx, const UINT lightIdx)
	{	
		std::string name = type;
	
		Entity11* entity = new Entity11(type, idx, pLight, lightIdx);
	...
	void Scene11::Draw()
	{
		...
		if (ImGui::Begin("Scene11 Hierarchy"))
		{
			if (ImGui::TreeNode(m_name.c_str()))
			{
				...
				//i:Entitiy배열, meshIdx:모델 그룹 내 하위 모델 배열
				for (int i = 0; i < m_entites.size(); ++i)
				{	
					int entitiyIdx = m_entites[i]->GetEntityIndex();
					if (m_entites[i]->IsObject())
					{
						Object11* pObject = m_entites[i]->GetpObject();
						...
						if (ImGui::TreeNode(objectName.c_str()))
						{
							...
							for (int j = 0; j < pMeshes.size(); ++j)
							{
								...
								{	
									...
									m_pD3D11->PickSubMesh(objectIdx, j);
									m_popUpID = popUpID;
								...
								if (m_popUpID==popUpID&&ImGui::BeginPopup(m_popUpID.c_str()))
								{
									...
									if (ImGui::Selectable(actions))
									{
										...
										if (!m_pD3D11->GetNumMesh(objectIdx))
										{
											RemoveObject(objectIdx, entitiyIdx);
					...
					{	
						ImGui::PushID(i);
						Light11* pLight = m_entites[i]->GetLight();
						...
						if (ImGui::BeginPopup("Edit"))
						{
							...
							{	
								...
								RemoveLight(lightIdx, entitiyIdx, pLight);
								for (int j = i; j < m_entites.size(); ++j)
								{	
									m_entites[j]->DecreaseEntityIndex();
									if (!m_entites[j]->IsObject()&&
		...
		if (ImGui::Begin("Properties"))
		{
			...
			{	
				...
				{	
					bool bDirty = false;
					Mesh11* pMesh = reinterpret_cast<Mesh11*>(m_pSelectedEntity);
				...
				{
					Light11* pLight = reinterpret_cast<Light11*>(m_pSelectedEntity);
	...
	void Scene11::DrawVec3Control(const std::string& label, float*const vals)
	{
	...
	void Scene11::Pick(const float sx, const float sy, const int width, const int height)
	{
		...
		m_pSelectedEntity = nullptr;
		m_pD3D11->PickSubMesh(_UNSELECT, _UNSELECT);
		for (int i = 0; i < m_entites.size(); ++i)
		{	
			if (!m_entites[i]->IsObject())
			{
	...
	bool Scene11::RaySphereIntersect(const XMFLOAT3 o, const XMFLOAT3 dir, const float r, float* const hitDistance)
	{
	...
	void Scene11::RemoveSelectedMesh(const int ObjectIdx, const int meshIdx)
	{   
		m_pD3D11->RemoveMesh(ObjectIdx, meshIdx);
		m_pSelectedEntity = nullptr;
	}
	void Scene11::DrawLightControl(Light11* const pLight)
	{	
		...
		case eLIGHT_TYPE::DIR:
			...
			{
				((DirectionalLight11*)pLight)->UpdateLightSpaceMatrices();
			}
		...
		default:
			...
			if (pos3->x != copyPos3.x || pos3->y != copyPos3.y || pos3->z != copyPos3.z)
			{
				...
				{
				case eLIGHT_TYPE::CUBE:
					((CubeLight11*)pLight)->CreateShadowMatrices();
					break;
				case eLIGHT_TYPE::SPT:
					((SpotLight11*)pLight)->UpdateViewMat();
				}
		...
		*(pLight->GetSpecular()) = DirectX::XMLoadFloat4(&specular4);
		...
		{	
			ImGui::Image(reinterpret_cast<void*>(m_pShadowSrv), ImVec2(128.f, 128.f));
		}
		
		
	}
	void Scene11::DrawCubeLightControl(Light11*const pLight) 
	{
		CubeLight11* pCubeLight = reinterpret_cast<CubeLight11*>(pLight);
	...
	void Scene11::DrawSpotLightControl(Light11* const pLight)
	{
		
		SpotLight11* pSpotLight = reinterpret_cast<SpotLight11*>(pLight);
	...
	void Scene11::RemoveObject(const int objectIdx, const int entityIdx)
	{	
		m_pD3D11->RemoveObject(objectIdx);
		RemoveEntity(entityIdx);
	}
	void Scene11::RemoveEntity(const int i)
	{	
	...
	void Scene11::RemoveLight(const int lightIdx, const int entityIdx, Light11* const pLight)
	{	
		m_pD3D11->RemoveLight(lightIdx, pLight);
		RemoveEntity(entityIdx);
	}
	void Scene11::UnselectMesh()
	{
		m_pSelectedEntity = nullptr;
		m_pD3D11->PickSubMesh(_UNSELECT, _UNSELECT);
	}
