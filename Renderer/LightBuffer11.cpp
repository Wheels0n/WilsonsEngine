...
namespace wilson
{
	void LightBuffer11::UpdateDirLightMatrices(ID3D11DeviceContext* const pContext)
	{
		...
		HRESULT hr = pContext->Map(m_pDirLitMatricesCb, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
		assert(SUCCEEDED(hr));
		...
		pContext->Unmap(m_pDirLitMatricesCb, 0);
		pContext->PSSetConstantBuffers(3, 1, &m_pDirLitMatricesCb);
	}
	...
	void LightBuffer11::UpdateSpotLightMatrices(ID3D11DeviceContext* const pContext)
	{
		...
		HRESULT hr = pContext->Map(m_pSpotLitMatricesCb, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
		assert(SUCCEEDED(hr));
		...
		pContext->Unmap(m_pSpotLitMatricesCb, 0);
		pContext->PSSetConstantBuffers(4, 1, &m_pSpotLitMatricesCb);
	}
	...
	void LightBuffer11::UpdateLightBuffer(ID3D11DeviceContext* const pContext)
	{
		...
		HRESULT hr = pContext->Map(m_pLightPropertyCb, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
		assert(SUCCEEDED(hr));
		...
		pContext->Unmap(m_pLightPropertyCb,0);
		pContext->PSSetConstantBuffers(2, 1, &m_pLightPropertyCb);
	...
	{	
		m_pLightPropertyCb = nullptr;
		m_pDirLitMatricesCb = nullptr;
		m_pSpotLitMatricesCb = nullptr;
		...
		pDevice->CreateBuffer(&cbufferDesc, 0, &m_pLightPropertyCb);
		...
		pDevice->CreateBuffer(&cbufferDesc, 0, &m_pDirLitMatricesCb);
		...
		pDevice->CreateBuffer(&cbufferDesc, 0, &m_pSpotLitMatricesCb);
	...
	{	
		if (m_pLightPropertyCb != nullptr)
		{
			m_pLightPropertyCb->Release();
			m_pLightPropertyCb = nullptr;
		}

		if (m_pDirLitMatricesCb != nullptr)
		{
			m_pDirLitMatricesCb->Release();
			m_pDirLitMatricesCb = nullptr;
		}

		if (m_pSpotLitMatricesCb != nullptr)
		{
			m_pSpotLitMatricesCb->Release();
			m_pSpotLitMatricesCb = nullptr;
		}
