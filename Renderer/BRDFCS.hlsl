...
static const uint SAMPLE_CNT = 1024u;
cbuffer dimension
{
    float width;
    float height;
...
{    
    float2 rect = float2(width, height);
    float2 uv = DTid.xy / rect;
    uv.y  = 1.0f - uv.y;
    g_dst[DTid.xy] = float4(IntergrateBRDF(max(1e-5, uv.x), uv.y), 0.0f, 1.0f);
}