...
namespace wilson
{
		...
		XMMATRIX* pViewMat, XMMATRIX* pProjMat)
	{
		...
		HRESULT hr;
		{
			...
			m_matBufferCBV = cbvSrvGpuHandle;
	...
	void MatBuffer12::UpdateMatBuffer(ID3D12GraphicsCommandList* pCommandlist, bool bSpotShadowPass)
	{
		HRESULT hr;
		UINT8* pMatBuffer;
		...
		pCommandlist->SetGraphicsRootDescriptorTable(0, m_matBufferCBV);
		return;
	}
