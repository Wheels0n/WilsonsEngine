...
#define IMGUI_HAS_VIEWPORT          // Viewport11 WIP branch
...
struct ImGuiPlatformIO;             // Multi-viewport support: interface for Platform/Renderer11 backends + viewports to render
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_ViewportsEnable        = 1 << 10,  // Viewport11 enable flags (require both ImGuiBackendFlags_PlatformHasViewports + ImGuiBackendFlags_RendererHasViewports set by the respective backends)
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasVtxOffset  = 1 << 3,   // Backend Renderer11 supports ImDrawCmd::VtxOffset. This enables output of large meshes (64K+ vertices) while still using 16-bit indices.
    ...
    ImGuiBackendFlags_RendererHasViewports  = 1 << 12,  // Backend Renderer11 supports multiple viewports.
};
...
struct ImGuiIO
{
    ...
    // Viewport11 options (when ImGuiConfigFlags_ViewportsEnable is set)
    ...
    // Optional: Platform/Renderer11 backend name (informational only! will be displayed in About Window) + User data for backend/wrappers to store their own stuff.
...
struct ImGuiWindowClass
{
    ...
    ImGuiViewportFlags  ViewportFlagsOverrideSet;   // Viewport11 flags to set when a window of this class owns a viewport. This allows you to enforce OS decoration or task bar icon, override the defaults on a per-window basis.
    ImGuiViewportFlags  ViewportFlagsOverrideClear; // Viewport11 flags to clear when a window of this class owns a viewport. This allows you to enforce OS decoration or task bar icon, override the defaults on a per-window basis.
...
struct ImDrawData
{
    ...
    ImGuiViewport*      OwnerViewport;      // Viewport11 carrying the ImDrawData instance, might be of use to the renderer (generally not).
...
struct ImFontConfig
{
    ...
    unsigned int    FontBuilderFlags;       // 0        // Settings11 for custom font builder. THIS IS BUILDER IMPLEMENTATION DEPENDENT. Leave as zero if unsure.
...
enum ImGuiViewportFlags_
{
    ...
    ImGuiViewportFlags_NoRendererClear          = 1 << 8,   // Platform Window: Renderer11 doesn't need to clear the framebuffer ahead (because we will fill it entirely).
    ...
    ImGuiViewportFlags_CanHostOtherWindows      = 1 << 11,  // Viewport11 can host multiple imgui windows (secondary viewports are associated to a single window). // FIXME: In practice there's still probably code making the assumption that this is always and only on the MainViewport. Will fix once we add support for "no main viewport".
...
struct ImGuiViewport
{
    ...
    // Our design separate the Renderer11 and Platform backends to facilitate combining default backends with each others.
    // When our create your own backend for a custom engine, it is possible that both Renderer11 and Platform will be handled
...
struct ImGuiPlatformIO
{
    ...
    //
    // The functions are designed so we can mix and match 2 imgui_impl_xxxx files, one for the Platform (~window/input handling), one for Renderer11.
    // Custom engine backends will often provide both Platform and Renderer11 interfaces and so may not need to use all functions.
    // Platform functions are typically called before their Renderer11 counterpart, apart from Destroy which are called the other way.
    ...
    int     (*Platform_CreateVkSurface)(ImGuiViewport* vp, ImU64 vk_inst, const void* vk_allocators, ImU64* out_vk_surface); // (Optional) For a Vulkan Renderer11 to call into Platform code (since the surface creation needs to tie them both).

    // (Optional) Renderer11 functions (e.g. DirectX, OpenGL, Vulkan)
