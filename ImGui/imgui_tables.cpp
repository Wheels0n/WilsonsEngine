...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    const int table_idx = g.Tables.GetEntityIndex(table);
...
void    ImGui::EndTable()
{
    ...
    outer_window->DC.CurrentTableIdx = g.CurrentTable ? g.Tables.GetEntityIndex(g.CurrentTable) : -1;
...
void ImGui::TableRemove(ImGuiTable* table)
{
    ...
    ImGuiContext& g = *GImGui;
    int table_idx = g.Tables.GetEntityIndex(table);
...
void ImGui::TableGcCompactTransientBuffers(ImGuiTable* table)
{
    ...
    g.TablesLastTimeActive[g.Tables.GetEntityIndex(table)] = -1.0f;
}
